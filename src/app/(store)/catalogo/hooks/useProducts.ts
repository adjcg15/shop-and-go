...
export function useProducts() {
  const [productsList, setProductsList] = useState<ProductsListState>(INITIA_PRODUCTS_LIST_STATE);
  ...
  const finishProductsLoading = useCallback((products: ProductWithInventory[], stillProducsToLoad: boolean = false) => {
    setProductsList(previousProductsList => ({ 
      loading: false,
  ...
  const fireErrorLoadingProducts = useCallback((message?: string) => {
    setProductsList(previousProducstList => ({ 
      ...
      error: message ?? "Estamos teniendo problemas para cargar los productos, por favor intente más tarde.",
