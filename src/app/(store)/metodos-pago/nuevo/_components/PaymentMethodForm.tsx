...
export const PaymentMethodForm = () => {
  ...
  return !issuingBanks.error ? (
    <form onSubmit={handleSubmit}>
      <div className={`form-group ${errors.cardholderName ? "invalid" : ""}`}>
      ...
      <div className={`form-group ${errors.bankIssuer ? "invalid" : ""}`}>
        ...
        >
          ...
          {issuingBanks.value?.map((issuer) => (
            <option key={issuer.id} value={issuer.id} className="text-gray-800">
              {issuer.name}
            </option>
          ))}
        </select>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div
          className={`flex form-group flex-col ${
            errors.expirationMonth || errors.expirationYear ? "invalid" : ""
          } `}
        >
        ...
        <div
          className={`flex form-group flex-col ${errors.cvv ? "invalid" : ""}`}
        >
          <label>CVV</label>
          <input
            id="cvv"
    ...
    <div className="flex justify-center items-center h-full">
      <p className="text-center mt-36 text-2xl gap-8">{issuingBanks.error}</p>
    </div>
  );
};
